      ...
      <div className="bar-fixed">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={projectChartData} layout="vertical" margin={{ top: 4, right: 6, bottom: 8, left: 4 }}>
            ...
            />
            <Tooltip content={<ChartTooltip mode="project" />} cursor={{ fill: '#F8F5F2' }} />
  ...
  return (
    <>
      {viewportMode === 'desktop' ? (
        <DesktopLayout theme={theme} onToggleTheme={toggleTheme} />
      ) : viewportMode === 'tablet' ? (
        <TabletLayout
          theme={theme}
          onToggleTheme={toggleTheme}
          homeContent={(onExplore) => (
            <HomeScreen
              theme={theme}
              onToggleTheme={toggleTheme}
              onExplore={onExplore}
        ...
        <MobileExperience theme={theme} onToggleTheme={toggleTheme} />
      )}
    </>
  );
}
